...
function reelect(node) {
	...
	var log = node.log;
	return function _reelect(type, state, path) {
...
Election.prototype.watch = function watch(znode, callback) {
	...
	return zk.aw_get(znode, reelect(this), function(rc, msg, stat) {
		if (rc !== 0)
			return callback(new ZooKeeperError(rc, msg));
...
Election.prototype.vote = function vote(callback) {
	...
	return this.create(function createCallback(err, znode) {
		if (err)
			return callback(err);

		return self.list(function listCallback(err, children) {
			if (err)
				return callback(err);
